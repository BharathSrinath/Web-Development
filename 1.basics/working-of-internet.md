# Introduction
    ...
    Initially, networks were centrlaized. Lets say that you are at point A and you are sending information to B, C, D, E, F, etc. Now if is sabotaged, the network will collapse as B, C, D, E, F, etc. are not connected to each other rather all of them are connected to a central point (A).
    # To overcome this, a distributed networking system was invented. In essence, now the Internet can be defined as 'network of networks'. It works by using a technique called packet switching, and by relying on standardized networking protocols that all computers can interpret.
...
# How does the Internet work?   
    ...
    # Other information the packet contains are
        ...
        # Which transport protocol is being used (TCP, UDP, etc.)
            ...
            # To mitigate this possibility, packets are designed with an expiration called TTL
...
# Protocols
    #W Different computers use different hardware and software. So, to send data across all the computers they need to follow a standard protocol
