    ...
    return {
      ...
      addAlbum: builder.mutation({
        invalidatesTags: (result, error, user) => {
          ...
          // In wanting of better design what we done is modified the providesTags. Look below for more details.
      ...
      fetchAlbums: builder.query({
        providesTags: (result, error, user) => {
          ...
          // Above approach will give us one tag for one endPoint. So whenever we make a request, this is the tag that will be asscoiated with that request.
          // But what we have done below is, we have added one tag for each album + an additional tag for the user
          // Now once we remove an album, one of the tags will be invalidated based on album.id. Here are getting the album.id from response (result) of RTK query. 
...
// How do we overcome the above issue?
  ...
  // Since we are dealing with positional arguments here, we have to specify result and error even though we are making any use of them.
