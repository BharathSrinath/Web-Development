...
# JSX
    ...
    # JSX code is not standalone JS; it needs to be transformed into regular JS before it can be understood by browsers.(Which is done by tools like 'babel' behind the scenes of which you dont worry about)
...
# React Setup
    # You will have 2 folders and a separate file names package.json
        # 2 folders - public and src
            # public has index.html 
            # src has App.js, index.js and all the other js files that we will be creating.
                # index.js is the main file under which all the App.js file will be executed. This is the file that is linked to the index.html. We are not going to touch that.
                # All js files will be exported to App.js and App.js will inturn be exported to index.js  
...
# Import and Export
    ...
    # export componentName from "./fileName"
        ...
        # If you want to import/export multiple functions
            # import {componentName1, componentName2} from "./fileName"
            # export {componentName1, componentName2} from "./fileName"

# How a function is executed in JS? 
    # functionName(); - Right?
    # But in react, it is slightly different.
...
# Conventions:
    ...
    # Separate CSS for each component rather than importing their respective .js files
...
# Props
    ...
    # How the argument is passed? 
        # <functionName argName="value"/> in the App.js file 
            ...
            # If the value is a string, it should be enclosed in "". For evreything else like numbers, arrays, objects they should be enclosed in {}.
    ...
    # Key-props:
        ...
        # Usually when working with react, we are use data that we have fetched from API. In that case most of the data will have unique id's. So you can always assign the id;s to the keys. 
...
# State
    # React has two systems. Event and State. Event detects the action performed by the user like clicking, hovering, etc. State is used to update the content on the screen based on the event. 
        # Example: Typing into the form should update the input field, clicking “next” on an image carousel should change which image is displayed, clicking “buy” puts a product in the shopping cart. Components need to “remember” things: the current input value, the current image, the shopping cart. In React, this kind of component-specific memory is called state.
    ...
    # Example for 'state' in functional components (with hooks):
        ...
        <!-- When the useState function is called, react assigns the initial value to the 'pice of state'. When the event occurs (say a button is clicked), then the user defined function will be called under which we will place the setter function. Setter function will contain the updated values. When the setter function is executed, entire component will be re-rendered (along with its children if it had any) with the updated value -->
    ...
    # Objects and Arrays in a State:
        ...
        3. Elements can be added at any index by using the slice method available on all arays. Actually slice method can be used to add elements at the start or end of an array as well.
            Example:
                const [colors, setColors] = useState(['red', 'green']);
                ...
                const changeTitleById = (id, newTitle) => {
                    const updatedBooks = books.map((book) => {
                        if (book.id === id) {
                            ...
                            <!-- Whenever we initialize our state, react will have a refernce to the current version of the state. Whenver we update our state, react will have a refernce to the new version of the state. It will hold on to both these references. -->
                                ...
                                <!-- Keep in mind that whatever that is inside an array really doesn't matter. React keeps reference of the array only and not whats inside -->
                                    <!-- If the above point is true, then with this example, we have created a copy of an array with map(). So can't we just update the object inside the array using method operator? -->
                                        <!-- Yes. It will work. But it will introduce bugs in later stages. I still didn't uunderstand that completely. But just keep in that we can't directly update the object without creating copy. -->
                        }
                        return book;
                    });
    ...
    # Creating a local server:
        ...
        2. Add the following under "scripts" in package.json file
            "server": "json-server -p 3001 --watch db.json"
                ...
                # Here the second command will execute the above command.
