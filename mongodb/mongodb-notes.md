...
# Broad categories: SQL vs NoSQL
    ...
    2. No Structured Query Language (SQL):
        ...
        # MongDB, CouchDB, Neo4j, Cassandra, Redis
...
# Installation  and initial setup of MongoDB:
    ...
    # Type 'mongosh' in bash. This will connect to the server that we have installed in step 1 and the shell that you are seeing is a JS shell where you can type JS code.
...
# CRUD operations
    ...
    3. UPDATE:
        # db.dogs.updateOne({name: "Mac"}, {$set: {age: 8}, $currentDate: {lastModeified: true}})
            # Herefirst argument is the property that you can use to find the document
        ...
        #  db.dogs.updateMany({name: "Mac"}, {$set: {age: 8, breed: "Country dog"}, $currentDate: {lastModeified: true}})
            # There is no property called breed. But the above command will the new property
    4. DELETE:
        # db.dogs.deleteOne
        # db.dogs.deleteMany
...
# Finding/Querying in Detail:
    ...
    # db.inventory.find( { "size.uom": "in" } ) => Accessm to the nested object is using the method operator.
    # db.inventory.find( { "size.h": { $lt: 15 }, "size.uom": "in", status: "D" } ) => Anothe example for AND. Note that we dont explcitly mention the word AND in MongoDB unlike SQLite.
    ...
    # db.inventory.find( { tags: { $all: ["red", "blank"] } } ) => db.inventory.find( { tags: { $all: ["red", "blank"] } } )
...
3. Model.find(filter, projection, options, callback): Returns all documents that match the given filter. 
    Examples: 
        ...
        await MyModel.find({ name: /john/i }, 'name friends').exec(); // executes, name LIKE john and only selecting the "name" and "friends" fields
...
6. Model.findByIdAndRemove(id, options, callback): Similar to findByIdAndDelete(), it deletes the document with the provided id and returns the deleted document. 
