...
// 1. Inline Event Handlers:
    ...
    // Both are not recommended. But still we will learn about it.
    // Example: <button onclick = "alert ('You clicked me!'); alert ('Click me again')"> Click Me!</button>
        // Here onclick is an attribute of button element and JS is written inside the quotes where alert is a key word
            // Instead of onclick you can use 'dblclick' too and many more
...
// 3. addEventListener Method: Best Practice than the above 2
    ...
    // ou can use event delegation, attaching a single listener to a common ancestor of multiple elements and handling events for child elements. 
...
// Here, you see that both of them will be executed with a single click and no overridng takes place
...
// Note. Just keep this in mind. The name that is used to refer to the element/class/if in the HTML and the funcion name of the eventlistener cannot be same.
...
// Don't use click. Because in forms buttons are not the only clickable element. You can click the input text element too. So click here is not suitbale. Hence going with submit. 
    ...
    // const tweetInput = document.querySelectorAll('input')[1];
        ...
        // Can you see the issue here? Eventhough it doe the job, it is difficult without knowing their index position. We can't keep refering to console. This is where the follwoing method come into picture.  
    const usernameInput = tweetForm.elements.username; // element is a keyword here
    const tweetInput = tweetForm.elements.tweet;
        // In this method we are using the 'name attribute' of that element. 
    // When you console.log(tweetForm), you will child of form elements. This is followed many other objectEvents. One such objectEvent is called as 'elements'. Dont get confused by the name. Under this you will have the collection of elements which has a 'name attribute'. We have to look for 'name attribute' that corresponds to the the input element from which need to collect the data.
...
// For an example, if you have type "Bharth" for the first time and clicked outside text area element or moved to the next element (Just imagine a form) and noticed that you mispelled your name and cameback to change that, 'change' event will log that
...
// When you look at the HTML, you sill see that button in nested under paragraph which is inherently nested under section.
...
clicks1.addEventListener('click', function(e){
    ...
    e.stopPropagation();
    // When you click the button, you will see 3 alerts, But e-stopPropagation stops bublling event and contains within #click1
})
...
// Here the code is in such a way where a button is inside of div element. 
    // onclick the the div has been set to hide with the help of 'display: none' in css
...
container.addEventListener('click', function (e) {
    ...
    // hide is a class here, not a key word
})
