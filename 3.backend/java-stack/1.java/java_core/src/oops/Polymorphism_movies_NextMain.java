...
//		In this main we will deal with casting classes
		Polymorphism_movies_Movie movie = 
				Polymorphism_movies_Movie.getMovie("A", "Jaws");
        movie.watchMovie();
...
//        So in the below step we are type-casting it to "Adventure".
        ...
        Object comedy = 
        		Polymorphism_movies_Movie.getMovie("C", "Airplane");
...
//        Now this a feature in Java where using 'var' keyword can automatically 
//        determine the return type. It is known as "local variable type inference"
//        It allows the compiler to infer the type of a variable based on the 
//        initializer (right side of the assignment operator)
        var airplane = 
        		Polymorphism_movies_Movie.getMovie("C", "Airplane");
        airplane.watchMovie();
//		  So, compiler infers the type to be "Polymorphism_movies_Movie" during
//        compile-time and the variable 'airplane' takes that type. But during 
//        run-time type of the initializer changes to "Comedy". But remember, the 
//        variable's inferred remain same as "Polymorphism_movies_Movie". That will
//        not change. So during run time, line will look like, 
        //  Polymorphism_movies_Movie airplane = Comedy.getMovie("C", "Airplane");
...
//        Now the question is, is there a way to know the run-time objects name? Actually 
//        no. We cannot know them during the compile time. But there are 3 common ways to 
//        write logics based on the return type at run-time.
...
//        	Method 3: Simple old method which gets the name of the return type at run-time
