...
function AnimalShow({ type }) {
  ...
  return (
    <div className="animal-show" onClick={handleClick}>
      ...
      {/* You can't use svgMap.type here because method operator will look for a property called 'type'. To access a property dynamically based on the value of a variable, you should use bracket notation. */}
      <img
        className="heart"
        alt="heart"
        src={heart}
...
// If multiple state updates are scheduled in rapid succession, React groups them into a single update to avoid unnecessary re-renders which improves its efficiency. In such a case, if we depend on the previous state value there's a risk that the updates will not be applied in the order we expect. This can lead to unexpected behavior, particularly when dealing with asynchronous updates.
// To overcome this, 
  // setClicks(prevClicks => prevClicks + 1);
  // Here we are using functional form of the state updater in React. 
