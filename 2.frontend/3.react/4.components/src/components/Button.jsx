...
// With classnames, you are not guaranteed the above rule. Sometimes it happens, sometimes it doesn't. It sometimes depends on kind of tool that you have used to create the react project (vite, create-react-app, etc.) 
// To make sure the rule is obeyed, you have enclose the class with twMerge. 
...
function Button({
  ...
  rounded,
  ...rest
  // Any props apart from the above willbe considered rest of the properties. Again a reminder. 'rest' is not a keyword.
  // Not related what we are learning but still. You know that both rest and spread operators are are preceded by 3 dots(...). Also both of them are not key words. SO how does JS differentiate between them?
    ...
    // The spread operator is used in array literals and function calls. It expands an iterable (like an array or string) into its elements1
...
// 
  ...
  return (
    <button {...rest} className={classes}>
      {/* So how does it work? We have the className called classes which is being passed to every button element. But all of the stylings inside the classes are not applied to the button. Rather it is dependent on the prop that we are passing from the App component ofr each button.*/}
      {children}
    </button>
  );
}
