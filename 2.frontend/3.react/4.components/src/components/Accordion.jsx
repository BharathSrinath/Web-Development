...
function Accordion({ items }) {
  ...
  // Items being an array, we are making use of their indices. They start from 0. When we click the 0th index (which will expand), other indices (like 1,2,3,etc.) should collapse. Hence, we are seeting the defualt indice value to be -1.
  ...
  const handleClick = (nextIndex) => {
    setExpandedIndex((currentExpandedIndex) => {
      ...
      if (currentExpandedIndex === nextIndex) {
        // Checking whether the use clicks teh index that is already expanded. If that is the we to collpase by setting the default value (-1).
        return -1;
      } else {
        return nextIndex;
      }
  ...
  const renderedItems = items.map((item, index) => {
    ...
    const icon = (
      <span className="text-2xl">
        ...
        {/* We are making use of the isExpanded variable to display different icons depending whether the element is collapsed or expanded */}
      </span>
    );
...
// Important note: React doesn't print booleans, nulls and undefined