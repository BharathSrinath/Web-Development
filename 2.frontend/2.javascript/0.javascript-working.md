...
# Execution of JavaScript
    ...
    # Lexical Environment:
        # From the memory management and execution point of view, the two execution context is said to have variable environment and thread of execution.
...
# Block Scope in JS:
    ...
    # They are used group statements together so that we can use multiple statements in a place where JS expects one statement.
...
# JS Environment:
    # JS Environment encompasses JS Engine within which we have Heap and Call Stack.
    # Whenever a JS file is executed, a global execution context is created and puhsed on to the stack.  
    # JS Environment also has Web API's, Event Loop, Task/Callbacl Queue and Microtask queue.

# Heap:
    # We have already studied about call stack. Lets look about heap.
    # The heap is a region of memory used for dynamic memory allocation. In JS, the heap is where objects and reference types (like arrays, functions, and objects) are stored.
...
# Web API's: 
    ...
    # You will be surprised to know that fetch, setTimeout, addEventListener, etc. are not part of JS rather they are features provided by JS.
    # Types of Web API's:    
        # 1. DOM API: 
...
# Micro-task Queue:
    ...
    # While network-related requests, such as those made with fetch, result in callbacks that are placed in the macro-task queue, the callbacks associated with their Promises are placed in the micro-task queue. Similarly, MutationObservers, which monitor changes in the DOM tree, register their callback functions in the micro-task queue.
...
# JS Engine:
    ...
    # 1. Parsing: 
        ...
        # These tokens are converted to Abstract Syntax Tree by a Syntax Parser. An AST is a hierarchical tree structure that represents the syntactic structure of source code.
    # 2. Compilation and 3. Execution:
        ...
        # Generally a programming language can be interpreted language or a compiled language
            # Interprter: Executes the program line by line. Hence very fast but not efficient.
